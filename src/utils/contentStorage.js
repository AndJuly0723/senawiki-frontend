import { heroes as baseHeroes } from '../data/heroes'
...
const normalizeHero = (hero) => {
  ...
  return {
    ...
    typeLabel: hero?.typeLabel ?? typeMeta?.label ?? '미분류',
    typeIcon: hero?.typeIcon ?? typeMeta?.icon ?? '',
    gradeLabel: hero?.gradeLabel ?? gradeMeta?.label ?? '미분류',
...
const normalizePet = (pet) => {
  ...
  return {
    ...
    gradeLabel: pet?.gradeLabel ?? gradeMeta?.label ?? '미분류',
